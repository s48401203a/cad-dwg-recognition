...
月台高度、库墙高、屋脊高、设备间距/净空、托盘判定阈值、月台到车尾距离、车型图例计数
等**可能针对特定场地**的数值**不在公共代码与公共配置中**，由本机私有配置提供
（`site-adaptations.yaml` 的 `site_values` / `vehicle_legend_counts`）。

未配置时的影响（如实说明，不会静默套用任何内置数值）：

| 能力 | 未配置时的表现 |
|---|---|
| 规则引擎的场地尺寸 | 语义结果中**不写入** `dock_height_m` / `warehouse_wall_height_m` / 屋脊高等字段 |
| 电力设备净空与间距推断 | **不套用**：不做 UPS/电池柜的位置重排，也不写净空属性 |
| 托盘/桥架阈值 | **不设阈值**（不做最小段长过滤与标签聚类容差） |
| 月台到车尾距离等 | 相应字段不写入结果 |
| 车型序列分配 | 按**等权**处理，而不是某个场地的图例计数 |
| 摆放试验场 | 使用代码中明确标注的**通用占位值**（库墙 6.0 m / 屋脊 7.0 m / 月台 1.2 m），
并在 `height_source` 中标记为 `placeholder_default`，便于区分真实配置 |

需要这些能力时，按模板配置本机私有目录即可恢复（`backend/config/examples/site-adaptations.example.yaml`）。
