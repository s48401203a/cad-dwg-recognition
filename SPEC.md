# DWG 图纸识别 + Three.js 3D 预览工具 — 技术方案（历史文档）

> **⚠️ 历史文档，仅作设计参考，不代表当前实现。**
> 本文档写于项目早期，其中 Vue/Pinia 前端方案、部分目录结构与接口约定**均已作废**：
> 当前实现是原生 JS + Three.js（无构建步骤，由 FastAPI 直接提供静态页面），
> 后端保持 Python + FastAPI + ezdxf。请以 `README.md` 与 `docs/generic-cad-eval.html`
> 描述的实际行为为准；不要按本文档迁移现有前端或回退已有能力。
>
